...
export default function Payment() {
  ...
  const handlePayment = async () => {
    ...
    if (selectedMethod === "nfc") {
      setShowNFC(true);
      // Simulate NFC payment processing time
  ...
  const processNFCPayment = async (paymentAmount) => {
    setIsProcessing(true);
    const newBalance = user.account_balance - paymentAmount;

    await User.updateMyUserData({ account_balance: newBalance });

    await Transaction.create({
      type: "nfc_payment",
      amount: paymentAmount,
      description: description || "Pagamento por aproximação",
      recipient: "Estabelecimento",
      status: "completed",
      category: "shopping",
  ...
  const processPayment = async (paymentAmount) => {
    ...
    const transactionType = selectedMethod === "pix" ? "pix" : "transfer";

    await Transaction.create({
      type: transactionType,
      amount: paymentAmount,
      description: description || `${selectedMethod === "pix" ? "PIX" : "Transferência"} para ${recipient}`,
      recipient: recipient,
      status: "completed",
      category: "other",
      balance_after: newBalance,
